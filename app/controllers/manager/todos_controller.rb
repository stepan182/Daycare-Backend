class Manager::TodosController < ApplicationController
  ...
  def create
    ...
    def edit_index
        @todos = Todo.all.joins(:customers).where(customers: {customer_type_id: 1, id: session[:customer_id]}).distinct
    end
    ...
    def report_index
        @todos = Todo.all.joins(:customers).where(customers: {customer_type_id: 1, id: session[:customer_id]}).joins(:user_types).where(user_types: {id: params["user_type_name"]["user_type_id"]}).joins(:daycare_departments).where(daycare_departments: {id: params["daycare_department_name"]["daycare_department_id"]}).distinct
    end
